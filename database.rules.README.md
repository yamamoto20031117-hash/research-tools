...
**クライアント側のパスワードは原理的に無意味**（ソースを見れば書いてある）。
実際の管理権限は DB ルール側の `admins` ノードで持つ:

```
/admins/<Firebase認証uid>: true
```

ここに載っている uid だけが、他人のスレッド・レスを削除できる。`admins` 自体は
ルール上どこからも書けない（`.write: false`）ので、**Firebase コンソールから
手で追加する**こと。自分の uid は、匿名認証を有効化したあとブラウザの
コンソールで `firebase.auth().currentUser.uid` を実行すれば分かる。

注意: 匿名認証の uid は**ブラウザのストレージを消すと変わる**。恒久的に管理者を
持ちたいなら、匿名ではなく Google ログインなどに変えて、その uid を登録する。
