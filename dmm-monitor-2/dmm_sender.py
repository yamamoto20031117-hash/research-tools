...
FIREBASE_PATH_LIVE = "dmm2/live"      # リアルタイム値（最新1件を上書き）
FIREBASE_PATH_LOG = "dmm2/log"        # ログ（追記）
...
def update_output_status(on):
    ...
    firebase_put("dmm2/status", {"output": on, "time": int(time.time()*1000)})
...
def firebase_command_thread(smu):
    ...
    while running:
        try:
            # コマンドチェック
            cmd = firebase_get("dmm2/command")
            if cmd and cmd.get("action"):
                action = cmd["action"]
                firebase_delete("dmm2/command")
